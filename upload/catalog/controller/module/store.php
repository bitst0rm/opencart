	...
	protected function index() {
		...
		if ($status) {
			...
			$this->data['stores'][] = array(
				...
				'url'      => $this->config->get('config_secure') ? HTTPS_SERVER : HTTP_SERVER . 'index.php?route=common/home&session_id=' . $this->session->getId()
			);
			...
			foreach ($results as $result) {
				$this->data['stores'][] = array(
					...
					'name'     => $result['name'],
					'url'      => $this->config->get('config_secure') ? $result['ssl'] : $result['url'] . 'index.php?route=common/home&session_id=' . $this->session->getId()
				);
			}
